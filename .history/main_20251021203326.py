...
def main():
    ...
    # 存储所有数据的缓存（避免频繁写入文件）
    data_cache = []
    cache_threshold = 6*24 # 当缓存的数据条数达到这个阈值时，写入文件
    ...
    while True:
        ...
        for station in charge_stations:
            ...
            if data:
                ...
                round_data_count += len(data)

            # 微小时间间隔，避免被识别为恶意请求
            time.sleep(random.uniform(0.4, 0.8))  # 0.5到1秒之间的随机等待

        # 更新已获取的数据条数
        total_data_count += round_data_count

        # 每轮查询后输出当前状态
        round_elapsed_time = time.time() - round_start_time
        print(f"Round complete. Total data collected: {total_data_count} entries.")
        print(f"Time spent in this round: {round_elapsed_time:.2f} seconds.")
        print(f"Total runtime: {time.time() - start_time:.2f} seconds.")

        # 批量写入文件（如果缓存达到一定数量）
        if len(data_cache) >= cache_threshold:
            save_results_to_file(data_cache)
            print(f"Batch saved to file. Current total data collected: {total_data_count} entries.")
            data_cache.clear()  # 清空缓存
    
        # 每15秒后再进行下一轮查询，加入随机扰动
        elapsed_time = time.time() - round_start_time
        base_wait_time = 15  # 基本等待时间10秒
        random_wait_time = random.uniform(-2, 2)  # 加入-2到+2秒的随机扰动
        wait_time = max(0, base_wait_time - elapsed_time + random_wait_time)  # 确保时间不为负值
