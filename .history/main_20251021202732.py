...
def save_result_to_file(data):
    ...
    try:
        with open(filename, "r", encoding="utf-8") as f:  # 打开文件时使用utf-8编码
            results = json.load(f)
    ...
    with open(filename, "w", encoding="utf-8") as f:  # 保存时也使用utf-8编码
        json.dump(results, f, indent=4, ensure_ascii=False)  # `ensure_ascii=False` 保证中文字符不被转义
